...
pub enum Mediation {
    ...
    // Silent,
    // /// If we can get creds without the user having to do anything, great, other wise ask the
    // user. Probably a bad idea. Optional,
...
impl PublicKeyCredential {
    /// Retrieve the user uniqueid that *may* have been provided by the
    /// authenticator during this authentication.
...
impl From<web_sys::PublicKeyCredential> for PublicKeyCredential {
    fn from(data: web_sys::PublicKeyCredential) -> PublicKeyCredential {
        ...
        let data_raw_id_b64 = data_raw_id;
        let data_response_client_data_json_b64 = data_response_client_data_json;
        let data_response_authenticator_data_b64 = data_response_authenticator_data;
        let data_response_signature_b64 = data_response_signature;

        let data_response_user_handle_b64 = data_response_user_handle;
