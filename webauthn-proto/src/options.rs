...
///
/// ⚠️  WARNING - discouraged is marked with a warning, as in some cases, some
/// authenticators will FORCE verification during registration but NOT during
/// authentication. This means that is is NOT possible assert verification has
/// been bypassed or not from the server viewpoint, and to the user it may
/// create confusion about when verification is or is not required.
#[derive(
    Clone,
    Copy,
    Debug,
...
pub enum UserVerificationPolicy {
    ...
    /// verification, it may not be usable with this policy.
    Required,
    /// TO FILL IN
...
pub struct User {
    ...
    /// NEVER be changed. If in doubt, use a UUID.
    pub id: Vec<u8>,
...
pub enum AuthenticatorAttachment {
    ...
    Platform,
    /// Request a device that can be seperated from the machine aka an external
...
#[derive(
    Debug, Default, Serialize, Clone, Deserialize, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
)]
#[archive_attr(derive(bytecheck::CheckBytes))]
#[serde(rename_all = "camelCase")]
