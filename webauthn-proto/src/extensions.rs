//! Extensions allowing certain types of authenticators to provide supplemental
//! information.
...
pub struct RequestRegistrationExtensions {
    ...
    /// ⚠️  - Browsers support the *creation* of the secret, but not the
    /// retrieval of it. CTAP2.1 create hmac secret
    #[serde(skip_serializing_if = "Option::is_none")]
...
#[derive(
    Debug, Deserialize, Serialize, Clone, Default, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
)]
#[archive_attr(derive(bytecheck::CheckBytes))]
...
#[derive(
    Debug, Deserialize, Serialize, Clone, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
)]
#[archive_attr(derive(bytecheck::CheckBytes))]
...
pub struct RegistrationExtensionsClientOutputs {
    ...
    /// property is managed by the webbrowser, and is NOT SIGNED and CAN NOT be
    /// trusted!
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    T: Clone + std::fmt::Debug,
{
    ...
    Set(T),
    /// The extension was not requested, and the authenticator sent an
    /// unsolicited extension value.
    Unsolicited(T),
    /// ⚠️  WARNING: The data in this extension is not signed cryptographically,
    /// and can not be trusted for security assertions. It MAY be used for
    /// UI/UX hints.
    Unsigned(T),
}
