...
pub struct AppleAnonymousNonce;

pub struct AndroidKeyAttestationExtensionData;
...
pub mod android_key_attestation {
...
impl AttestationX509Extension for AndroidKeyAttestationExtensionData {
    // If cert contains an extension with OID 1.3.6.1.4.1.11129.2.1.17 (android key
    // attestation)
...
impl AttestationX509Extension for AppleAnonymousNonce {
    ...
    // 1.2.840.113635.100.8.2 ) in credCert. The nonce here is used to prove that
    // the attestation is live and to protect the integrity of the authenticatorData
    // and the client data.
...
// Perform the Verification procedure for 8.2. Packed Attestation Statement
// Format https://w3c.github.io/webauthn/#sctn-packed-attestation
pub fn verify_packed_attestation(
    ...
    match (
        ...
        (Some(x5c), _) => {
            ...
            // The elements of this array contain attestnCert and its certificate chain,
            // each encoded in X.509 format. The attestation certificate
            // attestnCert MUST be the first element in the array.
        ...
        (None, None) => {
            ...
            // 4.a. Validate that alg matches the algorithm of the credentialPublicKey in
            // authenticatorData.
            if alg != credential_public_key.type_ {
                return Err(WebauthnError::AttestationStatementAlgMismatch);
            }

            // 4.b. Verify that sig is a valid signature over the concatenation of
            ...
            // 4.c. If successful, return implementation-specific values representing
...
pub fn verify_fidou2f_attestation(
    acd: &AttestedCredentialData,
    att_obj: &AttestationObject<Registration>,
    client_data_hash: &[u8],
    ...
    //
    // ^-- This is already DONE as a factor of serde_cbor_2 not erroring up to this
    // point, and those errors will be handled better than just "unwrap" :)
...
pub fn verify_tpm_attestation(
    ...
    // A COSEAlgorithmIdentifier containing the identifier of the algorithm used to
    // generate the attestation signature. String("alg"): I64(-65535),
    let alg_value = att_stmt_map
        .get(&serde_cbor_2::Value::Text("alg".to_string()))
        .ok_or(WebauthnError::AttestationStatementAlgMissing)?;
    ...
    // The TPMS_ATTEST structure over which the above signature was computed, as
    // specified in [TPMv2-Part2] section 10.12.8. String("certInfo"):
    // Bytes([]),
    ...
    // The TPMT_PUBLIC structure (see [TPMv2-Part2] section 12.2.4) used by the TPM
    // to represent the credential public key. String("pubArea"): Bytes([]),
    ...
    // The attestation signature, in the form of a TPMT_SIGNATURE structure as
    // specified in [TPMv2-Part2] section 11.3.4. String("sig"): Bytes([]),
    let sig_value = att_stmt_map
        .get(&serde_cbor_2::Value::Text("sig".to_string()))
        .ok_or(WebauthnError::AttestationStatementSigMissing)?;
    ...
    // pubArea is identical to the credentialPublicKey in the
    ...
    match (
        ...
        (
            ...
            // Is it possible to check the exponent? I think it's not ... as the tpm_parms
            // and the cose rse disagree in my test vectors.
        ...
        (
            COSEKeyType::EcEc2(COSEEC2Key { curve, x, y }),
            ...
            if x.as_ref() != ecc_points.x || y.as_ref() != ecc_points.y {
    ...
    // Verify that extraData is set to the hash of attToBeSigned using the hash
    ...
    // using the procedure specified in [TPMv2-Part1] section 16. https://www.trustedcomputinggroup.org/wp-content/uploads/TPM-Rev-2.0-Part-1-Architecture-01.38.pdf
    ...
    match certinfo.typeattested {
        TpmuAttest::AttestCertify(name, _qname) => {
            ...
            // says nothing about validating them, so instead we prepend the bytes into the
            // hash so we do enforce these are checked
    ...
    // engines. https://www.trustedcomputinggroup.org/wp-content/uploads/TPM-Rev-2.0-Part-1-Architecture-01.38.pdf
    ...
    // public key in aik_cert with the algorithm specified in alg.
    ...
    // If successful, return implementation-specific values representing attestation
    // type AttCA and attestation trust path x5c.
    Ok((
...
pub fn verify_apple_anonymous_attestation(
    acd: &AttestedCredentialData,
    att_obj: &AttestationObject<Registration>,
    client_data_hash: &[u8],
) -> Result<(ParsedAttestationData, AttestationMetadata), WebauthnError> {
...
pub fn verify_android_key_attestation(
    ...
    // 3. Verify that the public key in the first certificate in x5c matches the
    ...
    // 4. Verify that the attestationChallenge field in the attestation certificate
    // extension data is identical to clientDataHash. The AuthorizationList.
    // allApplications field is not present on either authorization list
    // (softwareEnforced nor teeEnforced), since PublicKeyCredential MUST be scoped
    // to the RP ID. For the following, use only the teeEnforced authorization
    // list if the RP wants to accept only keys from a trusted execution
    // environment, otherwise use the union of teeEnforced and softwareEnforced.
    ...
    // 5. If successful, return implementation-specific values representing
    // attestation type Anonymous CA and attestation trust path x5c.
    Ok((ParsedAttestationData::Basic(arr_x509), meta))
}

/// <https://www.w3.org/TR/webauthn/#sctn-android-safetynet-attestation>
pub fn verify_android_safetynet_attestation(
    ...
    // 2. Verify that response is a valid SafetyNet response of version ver by
    // following the steps indicated by the SafetyNet online documentation. As
    // of this writing, there is only one format of the SafetyNet response and
    ...
    let (x5c, safetynet_response) =
        |token: &str| -> Result<(Vec<x509::X509>, SafteyNetAttestResponse), SafetyNetError> {
            ...
            // 3. Verify that the nonce attribute in the payload of response is identical to
            // the Base64 encoding of the SHA-256 hash of the concatenation of
            ...
            // 4. Verify that the SafetyNet response actually came from the SafetyNet
            // service by following the steps in the SafetyNet online documentation.
            let common_name = {
                let name = leaf_cert
                    .subject_name()
            ...
            // §8.5.6 Verify that the ctsProfileMatch attribute in the payload of response
            // is true.
...
pub fn verify_attestation_ca_chain<'a>(
    ...
    // If the ca_list is empty, Immediately fail since no valid attestation can be
    // created.
    ...
    // Note this is a result<result ... because the inner .init must return an
    // errorstack for openssl.
    let res: Result<_, _> = ca_ctx
        .init(&ca_store, leaf, &chain_stack, |ca_ctx_ref| {
            ca_ctx_ref.verify_cert().map(|_| {
                // The value as passed in is a boolean that we ignore in favour of the richer
                // error type.
                ...
                if res == x509::X509VerifyResult::OK {
                    ca_ctx_ref
                        ...
                        .and_then(|ca_cert| {
                            ...
                            // We let the digest bubble out now, we've done too
                            // much here already!
                        })
