...
/// This is the core of the Webauthn operations. It provides 4 interfaces that
...
///
/// Each of these is described in turn, but they will all map to routes in your
/// application. The generate functions return Json challenges that are intended
/// to be processed by the client browser, and the register and authenticate
/// will receive Json that is processed and verified.
///
/// These functions return state that you must store and handle correctly for
/// the authentication or registration to proceed correctly.
///
/// As a result, it's very important you read the function descriptions to
...
impl WebauthnCore {
    ...
    /// If possible, use the `webauthn-rs` crate, and it's safe wrapper instead!
    ///
    /// Webauthn as a standard has many traps that in the worst cases, may lead
    /// to bypasses and full account compromises. Many of the features of
    /// webauthn are NOT security policy, but user interface hints. Many
    /// options can NOT be enforced. `webauthn-rs` handles these correctly.
    /// USE `webauthn-rs` INSTEAD.
    ///
    /// If you still choose to continue, and use this directly, be aware that:
    ///
    /// * This function signature MAY change WITHOUT NOTICE and WITHIN MINOR
    ///   VERSIONS
    /// * That you are responsible for UPHOLDING many invariants within webauthn
    ///   that are NOT DOCUMENTED
    /// * You MUST understand the webauthn specification in excruciating detail
    ///   to understand the traps within it
    ///
    ...
    /// Same as `generate_challenge_register_options` but with simple, default
    /// options
    ...
    /// Generate a new challenge for client registration. This is the first step
    /// in the lifecycle of a credential. This function will return the
    ...
    /// persist. It is strongly advised you associate this RegistrationState
    /// with the UserId of the requester.
    ...
    pub fn generate_challenge_register_options(
        ...
        let c = CreationChallengeResponse {
            public_key: PublicKeyCredentialCreationOptions {
                ...
                challenge: challenge.clone().to_vec(),
    ...
    /// `navigator.credentials.create()` which is sent to the webserver from the
    /// client.
    ///
    /// Given the username you also must provide the associated
    /// RegistrationState for this operation to proceed.
    ///
    /// On success this returns a new Credential that you must persist and
    /// associate with the user.
    ///
    /// You need to provide a closure that is able to check if any credential of
    /// the same id has already been persisted by your server.
    pub fn register_credential(
        ...
        // Decompose our registration state which contains everything we need to
        // proceed.
        trace!(?state);
        trace!(?reg);
        ...
        // Check that the credentialId is not yet registered to any other user. If
        // registration is requested for a credential that is already registered
        // to a different user, the Relying Party SHOULD fail this registration
        // ceremony, or it MAY decide to accept the registration, e.g. while
        // deleting the older registration.
    ...
    pub fn register_credential_internal(
        ...
        // Internal management - if the attestation ca list is some, but is empty, we
        // need to fail!
        if attestation_cas
            .as_ref()
            .map(|l| l.is_empty())
        ...
        //
        //   This is done by the calling webserver to give us reg aka JSONText in this
        // case.
        //
        // Let C, the client data claimed as collected during the credential creation,
        // be the result of running an implementation-specific JSON parser on
        // JSONtext.
        //
        ...
        // Verify that the value of C.challenge matches the challenge that was sent to
        // the authenticator in the create() call.
        if data.client_data_json.challenge != chal.as_ref() {
        ...
        // Verify that the value of C.tokenBinding.status matches the state of Token
        // Binding for the TLS connection over which the assertion was obtained.
        // If Token Binding was used on that TLS connection, also verify that
        // C.tokenBinding.id matches the base64url encoding of the Token Binding
        // ID for the connection.
        //
        //  This could be reasonably complex to do, given that we could be behind a load
        // balancer or we may not directly know the status of TLS inside this
        // api. I'm open to creative suggestions on this topic!
        //

        // Let hash be the result of computing a hash over response.clientDataJSON using
        // SHA-256
        //
        //   clarifying point - this is the hash of bytes.
        //
        // First you have to decode this from base64!!! This really could just be
        ...
        // statement format fmt, the authenticator data authData,
        ...
        // matches the alg attribute of one of the items in
        ...
        // extension input values that were given as the extensions option in
        // the create() call. In particular, any extension identifier values in the
        // clientExtensionResults and the extensions in authData MUST be also be present
        // as extension identifier values in the extensions member of options,
        // i.e., no extensions are present that were not requested. In the
        ...
        // maintained in the IANA registry of the same name [WebAuthn-Registries]. ( https://www.rfc-editor.org/rfc/rfc8809 )
        //
        ...
        // attestation signature, by using the attestation statement format
        // fmt’s verification procedure given attStmt, authData and the hash of
        // the serialized client data.
        ...
        // source or from policy. For example, the FIDO Metadata
        // Service [FIDOMetadataService] provides one way to obtain such information,
        ...
        let attested_ca_crt = if let Some(ca_list) = attestation_cas {
            ...
            // It may seem odd to unwrap the option and make this not verified at this
            // point, but in this case because we have the ca_list and none was
            // the result (which happens) in some cases, we need to map that
            // through. But we need verify_attesation_ca_chain to still return
            // these option types due to re-attestation in the future.
        ...
        // If the attestation statement attStmt verified successfully and is found to be
        // trustworthy, then register the new credential with the account that
        ...
        // credential.response.getTransports(). This value SHOULD NOT be modified before
        // or after storing it. It is RECOMMENDED to use this value to populate
        // the transports of the allowCredentials option in future get() calls
    ...
    pub fn verify_credential_internal(
        ...
        // Let cData, authData and sig denote the value of credential’s response's
        ...
        */
        ...
        // Verify that the value of C.challenge matches the challenge that was sent to
        // the authenticator in the PublicKeyCredentialRequestOptions passed to
        // the get() call.
        if c.challenge != chal.as_ref() {
            return Err(WebauthnError::MismatchedChallenge);
        }
        ...
        // Verify that the value of C.tokenBinding.status matches the state of Token
        // Binding for the TLS connection over which the attestation was
        ...
        // Verify that the rpIdHash in authData is the SHA-256 hash of the RP ID
        // expected by the Relying Party. Note that if we have an appid stored
        // in the state and the client indicates it has used the appid extension,
        ...
        // If user verification is required for this assertion, verify that the User
        // Verified bit of the flags in authData is set.
        //
        // We also check the historical policy too. See
        // designs/authentication-use-cases.md
        ...
        match (&policy, &cred.registration_policy) {
            ...
            (_, UserVerificationPolicy::Preferred) => {
                // If we asked for Preferred at registration, we MAY have established to the
                // user that they are required to enter a pin, so we SHOULD
                // enforce this.
            ...
            // Pass - we can not know if verification was requested to the client in the past
            // correctly. This means we can't know what it's behaviour is at the moment.
            // We must allow unverified tokens now.
            _ => {}
        }

        // OUT OF SPEC - if the backup elligibility of this device has changed, this may
        // represent a compromise of the credential, tampering with the device,
        // or some other change to its risk profile from when it was originally
        // enrolled. Reject the authentication if this situation occurs.
        ...
        // clientExtensionResults and the authenticator extension outputs in the
        // extensions in authData are as expected, considering the client
        // extension input values that were given as the extensions option in the get()
        ...
        // options, i.e., no extensions are present that were not requested. In
        // the general case, the meaning of "are as expected" is specific to the Relying
        // Party and which extensions are in use.
        //
        // Note: Since all extensions are OPTIONAL for both the client and the
        // authenticator, the Relying Party MUST be prepared to handle cases
        ...
        // U2F authenticators. See §6.1.2 FIDO U2F Signature Format
        // Compatibility.
    ...
    /// Authenticate a set of credentials, deriving the correct user
    /// verification policy for them in a secure manner.
    pub fn generate_challenge_authenticate(
        ...
        //
        // https://github.com/valeralabs/apiv2/issues/91
        //
        let mut policies =
    ...
    /// Authenticate a single credential, with the ability to override the
    /// userVerification policy requested, or extensions in use. If
    /// userVerification is `None`, the policy from registration is used.
    ///
    /// NOTE: Over-riding the UserVerificationPolicy may have SECURITY
    /// consequences. You should understand how this interacts with the
    /// single credential in use, and how that may impact your system
    /// security.
    ///
    ...
    /// Authenticate a set of credentials allowing the user verification policy
    /// to be set.
    ///
    /// NOTE: Over-riding the UserVerificationPolicy may have SECURITY
    /// consequences. You should understand how this interacts with the
    ...
    /// Process an authenticate response from the authenticator and browser.
    /// This is the output of `navigator.credentials.get()`, which is
    /// processed by this function. If the authentication fails, appropriate
    /// errors will be returned.
    ///
    /// This requires the associated AuthenticationState that was created by
    /// generate_challenge_authenticate
    ///
    /// On successful authentication, an Ok result is returned. The Ok may
    /// contain the CredentialID and associated counter, which you *should*
    /// update for security purposes. If the Ok returns `None` then the
    /// credential does not have a counter.
    pub fn authenticate_credential(
        ...
        // If the allowCredentials option was given when this authentication ceremony
        // was initiated, verify that credential.id identifies one of the public
        // key credentials that were listed in allowCredentials.
        //
        // We always supply allowCredentials in this library, so we expect creds as a
        // vec of credentials that would be equivalent to what was allowed.
        // trace!("rsp: {:?}", rsp);

        let cred = {
            // Identify the user being authenticated and verify that this user is the owner
            // of the public key credential source credentialSource identified
            // by credential.id:
            //
            ...
            //      credential.response.userHandle is present, let userHandle be its value.
            // Verify that      userHandle also maps to the same user.

            //  If the user was not identified before the authentication ceremony was
            // initiated,      verify that credential.response.userHandle is
            // present, and that the user identified      by this value is the
            // owner of credentialSource.
            //
            //      Note: User-less mode is handled by calling
            // `AuthenticationState::set_allowed_credentials`      after the
            // caller extracts the userHandle and verifies the credential Source

            // Using credential’s id attribute (or the corresponding rawId, if base64url
            // encoding is inappropriate for your use case), look up the
        ...
        // inappropriate for your use case), look up the corresponding
        // credential public key and let credentialPublicKey be that credential
        // public key.

        // * Due to the design of this library, in the majority of workflows the user
        //   MUST be known
        // before we begin, else we would not have the allowed Credentials list. When we
        // proceed to allowing resident keys (client side discoverable) such as
        // username-less, then we will need to consider how to proceed here. For
        // now, username-less is such a hot-mess due to RK handling
        // being basicly non-existant, that there is no point. As a result, we have
        // already enforced these conditions.
        ...
        // stored in conjunction with credential’s id attribute is nonzero, then
        // run the following sub-step:
        if counter > 0 || cred.counter > 0 {
            // greater than the signature counter value stored in conjunction with
            // credential’s id attribute.       Update the stored signature
            // counter value, associated with credential’s id attribute,
            //       to be the value of authData.signCount.
            // less than or equal to the signature counter value stored in conjunction with
            // credential’s id attribute.      This is a signal that the
            // authenticator may be cloned, i.e. at least two copies      of the
            // credential private key may exist and are being used in parallel. Relying
            //      Parties should incorporate this information into their risk scoring.
            // Whether the      Relying Party updates the stored signature
            // counter value in this case, or not,      or fails the
...
mod tests {
    ...
    fn test_registration_packed_attestation() {
        ...
        // Example generated using navigator.credentials.create on Chrome Version
        // 77.0.3865.120 using Touch ID on MacBook running MacOS 10.15
        let rsp = r#"{
    ...
    fn test_authentication() {
        ...
        let cred = Credential {
            ...
            cred: COSEKey {
                ...
                key: COSEKeyType::EcEc2(COSEEC2Key {
            ...
            backup_state: false,
            registration_policy: UserVerificationPolicy::DiscouragedDoNotUse,
        ...
        // Captured authentication attempt, this mentions the appid extension has been
        // used, but we still provide a valid RPID
        let rsp = r#"
        {
            "id":"at-FfKGsOI21EhtCu7Vx-7t7FKkpUOyKXIkEBBD_vC-eym_AdW6Y9V8WyKxHmii11EBQEe7uFQ0bkYwb0GWmUQ",
            "rawId":"at-FfKGsOI21EhtCu7Vx-7t7FKkpUOyKXIkEBBD_vC-eym_AdW6Y9V8WyKxHmii11EBQEe7uFQ0bkYwb0GWmUQ",
    ...
    fn test_authentication_appid() {
        ...
        // Captured authentication attempt, this client has NOT used the appid
        // extension, but is providing the appid anyway
        let rsp = r#"
        {
            "id": "z077A6SzdvA3rDRG6tfnTf9TMFVtfLYe1mh27mRXgBZU6TXA_nCJAi6WnLLq1p3d0yj3n62_4yJMu80o4O8kkw",
            "rawId": "z077A6SzdvA3rDRG6tfnTf9TMFVtfLYe1mh27mRXgBZU6TXA_nCJAi6WnLLq1p3d0yj3n62_4yJMu80o4O8kkw",
        ...
        // This will verify against the RPID while the client provided an appid, so it
        // should fail
        let r = wan.verify_credential_internal(
            &rsp_d,
            UserVerificationPolicy::DiscouragedDoNotUse,
    ...
    fn test_registration_ipados_5ci() {
        ...
        // Assert this fails when the attestaion ca is correct, but the aaguid is
        // missing.
    ...
    fn test_uv_consistency() {
        ...
        // cred 1 verified + uv::dc
        {
            creds
                ...
                .map(|cred| {
                    cred.user_verified = true;
                    cred.registration_policy = UserVerificationPolicy::DiscouragedDoNotUse;
                })
                .unwrap();
            creds
                .get_mut(1)
    ...
    fn test_subdomain_origin() {
        ...
        let id = "zIQDbMsgDg89LbWHAMLrpgI4w5Bz5Hy8U6F-gaUmda1fgwgn6NzhXQFJwEDfowsiY0NTgdU2jjAG2PmzaD5aWA".to_string();
        let raw_id = vec![
    ...
    fn test_yk5bio_fallback_alg_attest_none() {
        ...
        let rsp_d: RegisterPublicKeyCredential = serde_json::from_str(
            r#"{
        ...
        debug!("{:?}", result);
        // Currently UNSUPPORTED as openssl doesn't have eddsa management utils that we
        // need.
    ...
    // ⚠️  Currently IGNORED as it appears that pixel 3a send INVALID attestation
    // requests.
    #[test]
    #[ignore]
    ...
    /// See https://github.com/valeralabs/apiv2/issues/105
    #[test]
    ...
    fn test_google_android_key() {
        let chal: Vec<u8> = serde_json::from_str("\"Tf65bS6D5temh2BwvptqgBPb25iZDRxjwC5ans91IIJDrcrOpnWTK4LVgFjeUV4GDMe44w8SI5NsZssIXTUvDg\"").unwrap();
