...
// - The openssl crate allows us to reconstruct a public key from the x/y group
//   coords, where most others want a pkcs formatted structure. As a result,
//   it's easiest to use openssl as it gives us exactly what we need for these
//   operations, and despite it's many challenges as a library, it has resources
//   and investment into it's maintenance, so we can a least assert a higher
//   level of confidence in it that <backyard crypto here>.

// Object({Integer(-3): Bytes([48, 185, 178, 204, 113, 186, 105, 138, 190, 33,
// 160, 46, 131, 253, 100, 177, 91, 243, 126, 128, 245, 119, 209, 59, 186, 41,
// 215, 196, 24, 222, 46, 102]), Integer(-2): Bytes([158, 212, 171, 234, 165,
// 197, 86, 55, 141, 122, 253, 6, 92, 242, 242, 114, 158, 221, 238, 163, 127,
// 214, 120, 157, 145, 226, 232, 250, 144, 150, 218, 138]), Integer(-1): U64(1),
// Integer(1): U64(2), Integer(3): I64(-7)})
//
...
impl<'a> TpmSanDataBuilder<'a> {
    ...
    pub fn manufacturer(mut self, value: &'a str) -> Self {
        self.manufacturer = Some(value);
        self
    }

    pub fn model(mut self, value: &'a str) -> Self {
    ...
    pub fn build(self) -> WebauthnResult<TpmSanData<'a>> {
        self.manufacturer
            .zip(self.model)
            .zip(self.version)
...
// pub const TCG_AT_TPM_MANUFACTURER: Oid = der_parser::oid!(2.23.133 .2 .1);
// pub const TCG_AT_TPM_MODEL: Oid = der_parser::oid!(2.23.133 .2 .2);
// pub const TCG_AT_TPM_VERSION: Oid = der_parser::oid!(2.23.133 .2 .3);

pub const TCG_AT_TPM_MANUFACTURER_RAW: &[u8] = &der_parser::oid!(raw 2.23.133 .2 .1);
pub const TCG_AT_TPM_MODEL_RAW: &[u8] = &der_parser::oid!(raw 2.23.133 .2 .2);
pub const TCG_AT_TPM_VERSION_RAW: &[u8] = &der_parser::oid!(raw 2.23.133 .2 .3);
...
pub fn assert_tpm_attest_req(x509: &x509::X509) -> Result<(), WebauthnError> {
    ...
    // [TPMv2-EK-Profile] section 3.2.9. https://www.trustedcomputinggroup.org/wp-content/uploads/Credential_Profile_EK_V2.0_R14_published.pdf
    check_extension(
    ...
    // CRL Distribution Point extension [RFC5280] are both OPTIONAL as the
    // status of many attestation certificates is available through metadata
...
pub fn assert_packed_attest_req(pubk: &x509::X509) -> Result<(), WebauthnError> {
    ...
    // Version MUST be set to 3 (which is indicated by an ASN.1 INTEGER with value
    // 2).
    if x509_cert.version != X509Version::V3 {
    ...
    //
    // Subject-C
    //  ISO 3166 code specifying the country where the Authenticator vendor is
    // incorporated (PrintableString) Subject-O
    ...
    // An Authority Information Access (AIA) extension with entry id-ad-ocsp and a
    // CRL Distribution Point extension [RFC5280] are both OPTIONAL as the
    // status of many attestation certificates is available through
    // authenticator metadata services. See, for example, the FIDO Metadata
...
pub fn only_hash_from_type(alg: COSEAlgorithm, _input: &[u8]) -> Result<Vec<u8>, WebauthnError> {
    match alg {
        COSEAlgorithm::InsecureRs1 => {
            // sha1
...
impl TryFrom<&serde_cbor_2::Value> for COSEKey {
    ...
    fn try_from(d: &serde_cbor_2::Value) -> Result<COSEKey, Self::Error> {
        ...
        //     Integer(-3): Bytes([48, 185, 178, 204, 113, 186, 105, 138, 190, 33, 160,
        // 46, 131, 253, 100, 177, 91, 243, 126, 128, 245, 119, 209, 59, 186, 41, 215,
        // 196, 24, 222, 46, 102]),     Integer(-2): Bytes([158, 212, 171, 234,
        // 165, 197, 86, 55, 141, 122, 253, 6, 92, 242, 242, 114, 158, 221, 238, 163,
        // 127, 214, 120, 157, 145, 226, 232, 250, 144, 150, 218, 138]),
        //     Integer(-1): U64(1),
        ...
        // })
        // Now each of these integers has a specific meaning, and you need to parse them
        // in order. First, value 1 for the key type.
        ...
        // match key_type {
        // 1 => {} OctetKey
        if key_type == (COSEKeyTypeId::EcEc2 as i128)
            && (type_ == COSEAlgorithm::ES256
                || type_ == COSEAlgorithm::ES384
                || type_ == COSEAlgorithm::ES512)
        {
            // This indicates this is an EC2 key consisting of crv, x, y, which are stored
            // in crv (-1), x (-2) and y (-3)
            ...
            // return it
            Ok(cose_key)
        } else if key_type == (COSEKeyTypeId::EcOkp as i128) && (type_ == COSEAlgorithm::EDDSA) {
            debug!(?d, "WebauthnError::COSEKeyInvalidType - EcOkp");
            ...
            let cose_key = COSEKey {
                type_,
                key: COSEKeyType::EcOkp(COSEOKPKey {
    ...
    fn try_from((alg, pubk): (COSEAlgorithm, &x509::X509)) -> Result<COSEKey, Self::Error> {
        let key = match alg {
            COSEAlgorithm::ES256 | COSEAlgorithm::ES384 | COSEAlgorithm::ES512 => {
                ...
                Ok(COSEKeyType::EcEc2(COSEEC2Key {
                    curve,
                    x: xbn.to_vec().into(),
                    y: ybn.to_vec().into(),
                }))
