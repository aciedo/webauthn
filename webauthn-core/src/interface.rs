...
//! are designed to allow persistance and should not change.
...
use rkyv::with::Skip;
use std::fmt;
use webauthn_proto::cose::*;
use webauthn_proto::extensions::*;
use webauthn_proto::options::*;
...
/// The in progress state of a credential registration attempt. You must persist
/// this in a server side location associated to the active session requesting
/// the registration. This contains the user unique id which you can use to
/// reference the user requesting the registration.
#[derive(
...
/// The in progress state of an authentication attempt. You must persist this
/// associated to the UserID requesting the registration.
#[derive(
    Debug, Clone, Serialize, Deserialize, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
)]
#[archive_attr(derive(bytecheck::CheckBytes))]
...
impl ECDSACurve {
    /// Returns the size in bytes of the coordinate components (x and y) for the
    /// specified curve
...
/// You will likely never need to interact with this value, as it is part of the
/// Credential API.
#[derive(
    Clone,
    Debug,
...
pub struct Credential {
    ...
    pub counter: Counter,
    /// The set of transports this credential indicated it could use. This is
    /// NOT a security property, but a hint for the browser and the user
    /// experience to how to communicate to this specific device.
    ...
    /// for it to exist between multiple hardware authenticators, or be backed
    /// up.
    ///
    ...
    /// component interacts with the device, i.e. an always verified
    /// authenticator vs one that can dynamically request it.
    ...
    /// The attestation certificate of this credential, including parsed
...
impl Credential {
    /// Re-verify this Credential's attestation chain. This re-applies the same
    /// process for certificate authority verification that occured at
    /// registration. This can be useful if you want to re-assert your
    /// credentials match an updated or changed ca_list from the time that
    /// registration occured. This can also be useful to re-determine
    /// certain properties of your device that may exist.
    ///
    /// # Safety
    /// Due to the design of CA infrastructure by certain providers, it is NOT
    /// possible to verify the CA expiry time. Certain vendors use CA
    /// intermediates that have expiries that are only valid for
    /// approximately 10 minutes, meaning that if we enforced time validity,
    /// these would false negative for their validity.
...
/// A legacy serialisation from version 3 of Webauthn RS. Only useful for
/// migrations.
...
/// Serialised Attestation Data which can be stored in a stable database or
/// similar.
...
/// The processed Attestation that the Authenticator is providing in it's
/// AttestedCredentialData. This metadata may allow identification of the device
/// and it's specific properties.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum AttestationMetadata {
    ...
    Packed {
        /// This is the unique id of the class/type of device. Often this id can
        /// imply the properties of the device.
        aaguid: Uuid,
    },
    ...
    Tpm {
        ...
        /// imply the properties of the device.
        aaguid: Uuid,
        /// The firmware version of the device at registration. It can NOT be
        /// determined if this updates later, which may require you to
        /// re-register the device if you need to enforce a version
        /// update.
    ...
    AndroidSafetyNet {
        ...
        /// A stricter verdict of device integrity. If the value of
        /// ctsProfileMatch is true, then the profile of the device running your
        /// app matches the profile of a device that has passed Android
        /// compatibility testing and has been approved as a Google-certified
        /// Android device.
        cts_profile_match: bool,
        /// A more lenient verdict of device integrity. If only the value of
        /// basicIntegrity is true, then the device running your app likely
        /// wasn't tampered with. However, the device hasn't necessarily passed
...
pub enum ParsedAttestationData {
    ...
    AttCa(Vec<x509::X509>),
    /// The credential is authenticated using an anonymization CA, and may
    /// provide a ca chain to validate to it's root.
    ...
    fn into(self) -> SerialisableAttestationData {
        match self {
            ...
            ParsedAttestationData::AttCa(chain) => SerialisableAttestationData::AttCa(
                // Vec<u8>(c.to_der().expect("Invalid DER")),
                chain
                    .into_iter()
                    .map(|c| c.to_der().expect("Invalid DER"))
                    .collect(),
    ...
    fn try_from(data: SerialisableAttestationData) -> Result<Self, Self::Error> {
        Ok(match data {
            SerialisableAttestationData::Basic(chain) => ParsedAttestationData::Basic(
                chain
                    .into_iter()
                    .map(|c| x509::X509::from_der(&c).map_err(WebauthnError::OpenSSLError))
...
#[derive(
    Debug, Serialize, Clone, Deserialize, rkyv::Archive, rkyv::Serialize, rkyv::Deserialize,
)]
#[archive_attr(derive(bytecheck::CheckBytes))]
...
/// A structure representing an Attestation CA and other options associated to
/// this CA.
///
/// Generally depending on the Attestation CA in use, this can help determine
/// properties of the authenticator that is in use.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(
...
pub struct AttestationCa {
    /// The x509 root CA of the attestation chain that a security key will be
    /// attested to.
    pub ca: x509::X509,
    /// If not empty, the set of acceptable AAGUIDS (Device Ids) that are
    /// allowed to be attested as trusted by this CA. AAGUIDS that are not
    /// in this set, but signed by this CA will NOT be trusted.
    pub aaguids: BTreeSet<Uuid>,
}
#[allow(clippy::from_over_into)]
...
impl TryFrom<SerialisableAttestationCa> for AttestationCa {
    ...
    fn try_from(data: SerialisableAttestationCa) -> Result<Self, Self::Error> {
        Ok(AttestationCa {
            ca: x509::X509::from_der(&data.ca).map_err(WebauthnError::OpenSSLError)?,
...
impl AttestationCa {
    ...
    /// Update the set of aaguids this Attestation CA allows. If an empty
    /// btreeset is provided then this Attestation CA allows all Aaguids.
    pub fn set_aaguids(&mut self, aaguids: BTreeSet<Uuid>) {
        self.aaguids = aaguids;
    }

    /// Update the set of aaguids this Attestation CA allows by adding this
    /// AAGUID to the allowed set.
    ...
    /// The yubico u2f root ca. Applies to all devices up to and including
    /// series 5.
    ...
    ///
    /// Not eligible for strict - many TPM's use SHA1 in signatures, which means
    /// they are potentially weak.
    ///
    /// In the future we may reject RS1 signatures, allowing this to be moved
    /// into the strict category.
    ...
    ///
    /// Not eligible for strict - hardware is difficult to interact with, low
    /// quality, and easy to break or destroy.
    ...
    #[allow(unused)]
    pub fn google_safetynet_ca_old() -> Self {
        AttestationCa {
...
pub struct AttestationCaList {
    ...
    pub cas: BTreeMap<Vec<u8>, AttestationCa>,
}
...
impl AttestationCaList {
    ...
    /// This is a list of CA's who's manufactured authenticators are of the
    /// highest quality and guarantees for users and RP's. These are devices
    /// that not only are secure, but user friendly, consistent, and
    /// correct.
    pub fn strict() -> Self {
        let mut new = Self::default();
