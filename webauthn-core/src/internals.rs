...
impl<'a> From<&'a Vec<u8>> for &'a ChallengeRef {
    fn from(d: &'a Vec<u8>) -> Self {
...
impl ChallengeRef {
    ...
    pub fn new(challenge: &[u8]) -> &ChallengeRef {
        // SAFETY
        // Because of #[repr(transparent)], [u8] is guaranteed to have the same
        // representation as ChallengeRef. This allows safe casting between
        // *const pointers of these types.
...
pub fn process_authentication_extensions(
...
impl From<&COSEAlgorithm> for i64 {
    fn from(c: &COSEAlgorithm) -> Self {
        match c {
            ...
            COSEAlgorithm::InsecureRs1 => -65535,
        }
    }
}
*/
...
pub enum TpmtSignature {
    ...
    /// A raw signature, verifyied by a cert + hash combination. May be
...
pub fn tpm_device_attribute_parser(i: &[u8]) -> nom::IResult<&[u8], &[u8; 8]> {
    let (i, _) = tag("id:")(i)?;
...
mod tests {
    ...
    fn deserialise_tpmt_public() {
        ...
        // to represent the credential public key.
        let data: Vec<u8> = vec![
    ...
    fn deserialise_tpmt_signature() {
        ...
        // specified in [TPMv2-Part2] section 11.3.4.
        let data: Vec<u8> = vec![
